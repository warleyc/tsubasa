...
export class MAdventQuestWorldComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('jhi-m-advent-quest-world div table .btn-danger'));
  title = element.all(by.css('jhi-m-advent-quest-world div h2#page-heading span')).first();
...
export class MAdventQuestWorldUpdatePage {
  pageTitle = element(by.id('jhi-m-advent-quest-world-heading'));
  saveButton = element(by.id('save-entity'));
  cancelButton = element(by.id('cancel-save'));
  setIdInput = element(by.id('field_setId'));
  numberInput = element(by.id('field_number'));
  nameInput = element(by.id('field_name'));
  symbolTypeInput = element(by.id('field_symbolType'));
  ...
  async setSymbolTypeInput(symbolType) {
    await this.symbolTypeInput.sendKeys(symbolType);
  }

  async getSymbolTypeInput() {
    return await this.symbolTypeInput.getAttribute('value');
  }
...
export class MAdventQuestWorldDeleteDialog {
  private dialogTitle = element(by.id('jhi-delete-mAdventQuestWorld-heading'));
  private confirmButton = element(by.id('jhi-confirm-delete-mAdventQuestWorld'));
