...
export class MCardLevelComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('jhi-m-card-level div table .btn-danger'));
  title = element.all(by.css('jhi-m-card-level div h2#page-heading span')).first();
...
export class MCardLevelUpdatePage {
  pageTitle = element(by.id('jhi-m-card-level-heading'));
...
export class MCardLevelDeleteDialog {
  private dialogTitle = element(by.id('jhi-delete-mCardLevel-heading'));
  private confirmButton = element(by.id('jhi-confirm-delete-mCardLevel'));
