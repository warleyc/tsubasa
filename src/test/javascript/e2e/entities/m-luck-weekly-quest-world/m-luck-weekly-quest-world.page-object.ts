...
export class MLuckWeeklyQuestWorldComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('jhi-m-luck-weekly-quest-world div table .btn-danger'));
  title = element.all(by.css('jhi-m-luck-weekly-quest-world div h2#page-heading span')).first();
...
export class MLuckWeeklyQuestWorldUpdatePage {
  pageTitle = element(by.id('jhi-m-luck-weekly-quest-world-heading'));
  ...
  clearLimitInput = element(by.id('field_clearLimit'));
  ...
  async setClearLimitInput(clearLimit) {
    await this.clearLimitInput.sendKeys(clearLimit);
  }

  async getClearLimitInput() {
    return await this.clearLimitInput.getAttribute('value');
  }
...
export class MLuckWeeklyQuestWorldDeleteDialog {
  private dialogTitle = element(by.id('jhi-delete-mLuckWeeklyQuestWorld-heading'));
  private confirmButton = element(by.id('jhi-confirm-delete-mLuckWeeklyQuestWorld'));
