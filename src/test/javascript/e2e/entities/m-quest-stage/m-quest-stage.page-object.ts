...
export class MQuestStageComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('jhi-m-quest-stage div table .btn-danger'));
  title = element.all(by.css('jhi-m-quest-stage div h2#page-heading span')).first();
...
export class MQuestStageUpdatePage {
  pageTitle = element(by.id('jhi-m-quest-stage-heading'));
  ...
  mquestworldSelect = element(by.id('field_mquestworld'));
  ...
  async mquestworldSelectLastOption(timeout?: number) {
    await this.mquestworldSelect
      .all(by.tagName('option'))
      .last()
      .click();
  }

  async mquestworldSelectOption(option) {
    await this.mquestworldSelect.sendKeys(option);
  }

  getMquestworldSelect(): ElementFinder {
    return this.mquestworldSelect;
  }

  async getMquestworldSelectedOption() {
    return await this.mquestworldSelect.element(by.css('option:checked')).getText();
  }
...
export class MQuestStageDeleteDialog {
  private dialogTitle = element(by.id('jhi-delete-mQuestStage-heading'));
  private confirmButton = element(by.id('jhi-confirm-delete-mQuestStage'));
