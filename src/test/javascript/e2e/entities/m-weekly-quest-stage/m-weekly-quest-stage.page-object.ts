...
export class MWeeklyQuestStageComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('jhi-m-weekly-quest-stage div table .btn-danger'));
  title = element.all(by.css('jhi-m-weekly-quest-stage div h2#page-heading span')).first();
...
export class MWeeklyQuestStageUpdatePage {
  pageTitle = element(by.id('jhi-m-weekly-quest-stage-heading'));
  ...
  mweeklyquestworldSelect = element(by.id('field_mweeklyquestworld'));
  ...
  async mweeklyquestworldSelectLastOption(timeout?: number) {
    await this.mweeklyquestworldSelect
      .all(by.tagName('option'))
      .last()
      .click();
  }

  async mweeklyquestworldSelectOption(option) {
    await this.mweeklyquestworldSelect.sendKeys(option);
  }

  getMweeklyquestworldSelect(): ElementFinder {
    return this.mweeklyquestworldSelect;
  }

  async getMweeklyquestworldSelectedOption() {
    return await this.mweeklyquestworldSelect.element(by.css('option:checked')).getText();
  }
...
export class MWeeklyQuestStageDeleteDialog {
  private dialogTitle = element(by.id('jhi-delete-mWeeklyQuestStage-heading'));
  private confirmButton = element(by.id('jhi-confirm-delete-mWeeklyQuestStage'));
